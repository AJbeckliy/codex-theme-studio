...
普通主题只改配置，不改公共 CSS。只有当所有主题都需要一种新能力时，才修改共享引擎。
...
解决方法：不要修改每个主题包，只修共享的 `renderer-inject.js` 或 `base-theme.css`。修一次，所有主题同时恢复。
...
现在新增主题时，我只需要准备视觉方向、四张素材和一个 `theme.json`。安装、启动、快捷方式、图标、验证和恢复都复用同一套引擎。
